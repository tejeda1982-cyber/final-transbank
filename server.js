...
app.use(express.static(__dirname)); // sirve archivos estáticos (CSS, imágenes, JS)
...
async function enviarCorreo(cliente, cotizacion) {
  ...
  try {
    ...
    // Correo interno (copia)
    await resend.emails.send({
      from: "contacto@tumotoexpress.cl",
      to: "tucorreo@tumotoexpress.cl",
      subject: "Copia cotización cliente",
      html: `
        <h2>Cliente: ${cliente.nombre}</h2>
        <p>Teléfono: ${cliente.telefono}</p>
        <p>Correo: ${cliente.correo}</p>
        <p>Total cotización: $${cotizacion.total}</p>
      `
    });
  } catch (err) {
